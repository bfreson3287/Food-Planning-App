...
const _mem = {};
const _ls = (function() {
try { const s = window[‘local’+‘Storage’]; s.setItem(’_t’,‘1’); s.removeItem(’*t’); return s; }
catch(e) { return null; }
})();
...
_mem[k] = v;
if (_ls) try { *ls.setItem(k, v); } catch(e) {}
}
function load(key) {
const k = ’afdp*’ + key;
try { const d = _ls ? _ls.getItem(k) : _mem[k]; return d ? JSON.parse(d) : null; }
catch(e) { return null; }
}
...
detail: pieceWarning || ’Yield from ’ + rawGrams + ’g raw x ’ + yf + ’ = ~’ + estimated + ‘g cooked’,
fixes: [
...
const s = state.settings;
const meatLines = meat.length ? meat.map(function(m) { return ’  - ’ + (m.qty ? m.qty + ’ ’ : ‘’) + m.item; }).join(’\n’) : ’  (none entered)’;
const produceLines = delivery.length ? delivery.map(function(d) { return ’  - ’ + (d.qty ? d.qty + ’ ’ : ‘’) + d.item; }).join(’\n’) : ’  (none entered)’;
const leftoverLines = leftovers.length ? leftovers.map(function(l) { return ’  - ’ + (l.qty ? l.qty + ’ ’ : ‘’) + l.item; }).join(’\n’) : ’  (none)’;
const pantryHave = state.pantry.filter(function(p) { return p.have; }).map(function(p) { return p.item; }).join(’, ‘) || ‘None recorded’;
const pantryMissing = state.pantry.filter(function(p) { return !p.have; }).map(function(p) { return p.item; }).join(’, ’) || ‘None’;

const feedbackLines = [];
const allKnownMeals = [].concat(
state.currentMealPlan ? [].concat(state.currentMealPlan.week1 || [], state.currentMealPlan.week2 || []) : [],
SEED_RECIPES
);
const seenIds = new Set();
allKnownMeals.forEach(function(meal) {
...
const recentCuisines = state.mealHistory.slice(-18).map(function(m) { return m.cuisine; }).filter(Boolean);
const cuisineCounts = recentCuisines.reduce(function(a, c) { a[c] = (a[c] || 0) + 1; return a; }, {});
const overused = Object.entries(cuisineCounts).filter(function(e) { return e[1] >= 3; }).map(function(e) { return e[0]; });

const mexicanRecent = state.mealHistory.filter(function(m) { return m.mexicanFlag && (Date.now() - m.date) < 14 * 24 * 60 * 60 * 1000; }).length;
...
? ‘\nAlready locked in – DO NOT duplicate these:\n’ + lockedTitles.map(function(t) { return ’  - ’ + t; }).join(’\n’) : ‘’;

const isRegen = slotsNeeded < 9;
const weekStructure = isRegen
? ‘Generate ’ + slotsNeeded + ’ new dinner recipe’ + (slotsNeeded > 1 ? ‘s’ : ‘’) + ’ – a JSON array of exactly ’ + slotsNeeded + ’ item’ + (slotsNeeded > 1 ? ‘s’ : ‘’) + ‘. Aim for variety in difficulty and cuisine.’
...
(overused.length ? ‘\nCUISINE ROTATION: Used heavily recently – use sparingly: ’ + overused.join(’, ‘) + ‘.’ : ‘’) +
...
return rawArray.map(function(r, i) {
...
const d1 = [‘Mon’,‘Tue’,‘Wed’,‘Thu’,‘Fri’];
const d2 = [‘Mon’,‘Tue’,‘Wed’,‘Thu’];
return {
week1: recipes.slice(0, 5).map(function(m, i) { m._day = d1[i]; m._week = 1; m._locked = false; m._sufficiency = calcLeftoverSufficiency(m); return m; }),
week2: recipes.slice(5, 9).map(function(m, i) { m._day = d2[i]; m._week = 2; m._locked = false; m._sufficiency = calcLeftoverSufficiency(m); return m; })
};
}
...
sufDetail += suf.fixes.map(function(f, fi) { return ‘<button class="fix-btn" onclick="applyFix(\'' + meal.id + '\',' + idx + ',' + weekNum + ',' + fi + ')">’ + f.label + ‘</button>’; }).join(’ ‘);
...
const temp = meals[idx]; meals[idx] = meals[newIdx]; meals[newIdx] = temp;
...
const locked1 = state.currentMealPlan.week1.filter(function(m) { return m._locked; });
const locked2 = state.currentMealPlan.week2.filter(function(m) { return m._locked; });
const unlockedCount = state.currentMealPlan.week1.filter(function(m) { return !m._locked; }).length +
state.currentMealPlan.week2.filter(function(m) { return !m._locked; }).length;
if (unlockedCount === 0) { toast(‘All meals are locked – unlock some first.’, ‘info’); return; }

const session = state.currentSession || {};
const lockedTitles = locked1.concat(locked2).map(function(m) { return m.title; });
...
const d2 = [‘Mon’,‘Tue’,‘Wed’,‘Thu’];
let ni = 0;
state.currentMealPlan.week1 = d1.map(function(day, i) {
const lk = locked1.find(function(m) { return m._day === day; });
if (lk) return lk;
const fr = newRecipes[ni++];
if (fr) { fr._day = day; fr._week = 1; fr._locked = false; fr._sufficiency = calcLeftoverSufficiency(fr); return fr; }
return state.currentMealPlan.week1[i];
});
state.currentMealPlan.week2 = d2.map(function(day, i) {
const lk = locked2.find(function(m) { return m._day === day; });
if (lk) return lk;
const fr = newRecipes[ni++];
if (fr) { fr._day = day; fr._week = 2; fr._locked = false; fr._sufficiency = calcLeftoverSufficiency(fr); return fr; }
...
meal.steps.push(‘Add drained chickpeas or cooked lentils to boost protein for leftovers.’);
}
...
const allMeals = [].concat(plan.week1, plan.week2).filter(Boolean);
let html = ‘<div class="recipe-list">’;
allMeals.forEach(function(meal) {
let ingredients = meal.ingredients.slice();
let steps = meal.steps.slice();
const fb = state.recipeFeedback[meal.id];
if (fb) {
if (fb.proteinMultiplier && meal.primaryProtein) {
const newRaw = Math.round((meal.primaryProtein.rawGrams || 0) * fb.proteinMultiplier);
if (newRaw > 0) ingredients = ingredients.map(function(ing) {
return ing.toLowerCase().includes(meal.primaryProtein.name.toLowerCase()) ? ing.replace(/\d+g/, newRaw + ‘g’) : ing;
});
}
if (fb.lessSalt) steps.push(‘Finishing step: Season to taste at the very end – add salt gradually and taste between additions.’);
if (fb.excludeIngredients && fb.excludeIngredients.length) fb.excludeIngredients.forEach(function(excl) {
ingredients = ingredients.filter(function(ing) { return !ing.toLowerCase().includes(excl.toLowerCase()); });
});
}
...
‘<div class="meal-meta"><span class="tag tag-cuisine">’ + meal.cuisine + ‘</span><span class="tag">’ + getDifficultyEmoji(meal.difficulty) + ’ ’ + getDifficultyLabel(meal.difficulty) + ‘</span><span class="tag tag-time">⏱ ’ + meal.cookTime + ’ min</span></div>’ +
‘<div class="recipe-section"><h4>Ingredients</h4><ul class="ingredients-list">’ + ingredients.map(function(i) { return ‘<li>’ + i + ‘</li>’; }).join(’’) + ‘</ul></div>’ +
‘<div class="recipe-section"><h4>Method</h4><ol class="steps-list">’ + steps.map(function(s) { return ‘<li>’ + s + ‘</li>’; }).join(’’) + ‘</ol></div>’ +
...
switchTab(‘recipes’);
setTimeout(function() { const el = document.getElementById(‘recipe-’ + recipeId); if (el) el.scrollIntoView({ behavior: ‘smooth’, block: ‘start’ }); }, 100);
}

function openCookMode(recipeId) {
const allMeals = [].concat(
(state.currentMealPlan && state.currentMealPlan.week1) || [],
(state.currentMealPlan && state.currentMealPlan.week2) || []
);
const meal = allMeals.find(function(m) { return m && m.id === recipeId; });
if (!meal) return;
...
if (check.checked) {
step.style.opacity = ‘0.5’; step.style.textDecoration = ‘line-through’;
...
const safeTitle = meal.title.replace(/\/g, ‘\\’).replace(/’/g, “\’”);
html += ‘<div class="rate-card" id="rate-' + meal.id + '">’ +
‘<h3>’ + meal.title + (fb.blacklisted ? ’ <span class="blacklist-badge">BLACKLISTED</span>’ : ‘’) + ‘</h3>’ +
‘<div class="star-rating">’ +
[1,2,3,4,5].map(function(s) { return ‘<span class="star-btn ' + (s <= rating ? 'active' : '') + '" onclick="setRating(\'' + meal.id + '\',' + s + ',\'' + safeTitle + '\')">★</span>’; }).join(’’) +
‘<span class="text-xs text-muted" style="margin-left:8px">’ + (rating ? rating + ‘/5’ : ‘Not rated’) + ‘</span></div>’ +
...
const excl = excludeIngredients.includes(short);
return ‘<option value=”’ + short + ‘” ’ + (excl ? ‘selected’ : ‘’) + ‘>’ + short + (excl ? ’ ✗’ : ‘’) + ‘</option>’;
}).join(’’) +
‘</select></label>’ +
(excludeIngredients.length ? ‘<div class="text-xs mt-2">Excluded: ’ + excludeIngredients.map(function(e) { return ‘<span class="tag">’ + e + ’ <span style="cursor:pointer" onclick="removeExclusion(\'' + meal.id + '\',\'' + e + '\')">✕</span></span>’; }).join(’ ’) + ‘</div>’ : ‘’) +
...
if (rating === 1) { fb.blacklisted = true; toast(‘Blacklisted – Claude will not suggest this again.’, ‘error’); }
...
fb.excludeIngredients = fb.excludeIngredients.filter(function(e) { return e !== ingredient; });
save(‘recipeFeedback’, state.recipeFeedback); renderRateNotes(); renderRecipes();
}
}
...
if (meatPhrases.some(function(p) { return notes.toLowerCase().includes(p); })) {
const fb = state.recipeFeedback[recipeId];
if (!fb.proteinMultiplier) fb.proteinMultiplier = 1;
fb.moreMeatCount = (fb.moreMeatCount || 0) + 1;
fb.proteinMultiplier = fb.moreMeatCount <= 1 ? Math.min(1.6, fb.proteinMultiplier + 0.25) : Math.min(1.6, fb.proteinMultiplier + 0.10);
...
save(‘recipeFeedback’, state.recipeFeedback); renderRateNotes(); toast(‘Removed from blacklist’, ‘success’);
}
}
...
function loadPantry() { state.pantry = load(‘pantry’) || DEFAULT_PANTRY.slice(); }
...
const tr = document.createElement(‘tr’);
tr.innerHTML = ‘<td>’ + item.item + ‘</td><td><span class="text-xs">’ + item.category + ‘</span></td>’ +
...
‘<td><button class="btn-secondary btn-small" onclick="togglePantryHave(' + idx + ')">’ + (item.have ? ‘Mark Out’ : ‘Mark Have’) + ’</button> ’ +
‘<button class="btn-danger btn-small" onclick="removePantryItem(' + idx + ')">✕</button></td>’;
tbody.appendChild(tr);
});
renderPantryAlerts();
}
...
const newStaples = state.pantry.filter(function(p) { return p._new; });
const restock = state.pantry.filter(function(p) { return p.low || !p.have; });
if (!newStaples.length && !restock.length) { alerts.style.display = ‘none’; return; }
alerts.style.display = ‘block’;
if (newStaples.length) { newBox.innerHTML = ‘<strong>New staples this fortnight:</strong> ’ + newStaples.map(function(p) { return p.item; }).join(’, ’); newBox.style.display = ‘block’; } else newBox.style.display = ‘none’;
if (restock.length) { restockBox.innerHTML = ‘<strong>Restock soon:</strong> ’ + restock.map(function(p) { return p.item; }).join(’, ’); restockBox.style.display = ‘block’; } else restockBox.style.display = ‘none’;
}
...
const allMeals = [].concat(state.currentMealPlan.week1, state.currentMealPlan.week2).filter(Boolean);
...
return name.replace(/s$/, ‘’).replace(/\s+/g, ‘*’).replace(/[^a-z0-9*]/g, ‘’);
}

const session = state.currentSession || {};
const onHandRaw = [].concat(session.delivery || [], session.meat || [], session.leftovers || []).map(function(i) { return normaliseKey(i.item.toLowerCase()); });
...
const val = ingredientMap[key];
const pantryMatch = state.pantry.find(function(p) {
const pk = normaliseKey(p.item.toLowerCase());
return key.includes(pk) || pk.includes(key.split(’*’)[0]);
});
if (pantryMatch && pantryMatch.have) return;
const inOnHand = onHandRaw.some(function(ohk) {
const ohb = ohk.split(’*’)[0];
const ib = key.split(’_’)[0];
return (ohb.length > 3 && (key.includes(ohb) || ohk.includes(ib))) || ohk === key;
});
if (inOnHand) return;
let note = ‘’;
if ([‘soy sauce’,‘sesame oil’,‘mirin’,‘fish sauce’,‘oyster sauce’,‘gochujang’].some(function(a) { return val.name.includes(a); })) note = ‘Asian aisle at Woolies’;
else if ([‘cream’,‘yoghurt’,‘sour cream’,‘butter’,‘parmesan’,‘egg’].some(function(a) { return val.name.includes(a); })) note = ‘Dairy section’;
...
if (val.mentions > 1) qtyStr += ’ (x’ + val.mentions + ’ recipes)’;
...
list.forEach(function(item) { html += ‘<tr><td>’ + item.item + ‘</td><td>’ + item.qty + ‘</td><td>’ + item.notes + ‘</td></tr>’; });
html += ‘</tbody></table>’;
...
const plain = list.map(function(i) { return (i.qty ? i.qty + ’ ’ : ‘’) + i.item + (i.notes ? ’ (’ + i.notes + ‘)’ : ‘’); }).join(’\n’);
document.getElementById(‘wooliesPlainText’).textContent = plain;
document.getElementById(‘wooliesJSON’).textContent = JSON.stringify(list.map(function(i) { return { name: i.item, quantity: i.qty || ‘1’, notes: i.notes, preferred_brand: ‘’ }; }), null, 2);
...
if (!list || !list.length) return;
const D = docx;
const thinBorder = { style: D.BorderStyle.SINGLE, size: 1, color: ‘CCCCCC’ };
const borders = { top: thinBorder, bottom: thinBorder, left: thinBorder, right: thinBorder };
const headerRow = new D.TableRow({ children: [‘Item’,‘Quantity’,‘Notes’].map(function(text) {
return new D.TableCell({ children: [new D.Paragraph({ children: [new D.TextRun({ text: text, bold: true, size: 20, font: ‘Calibri’ })] })], borders: borders, shading: { fill: ‘F5F0E8’ }, width: { size: text === ‘Item’ ? 4000 : 2000, type: D.WidthType.DXA } });
})});
const rows = list.map(function(item) {
return new D.TableRow({ children: [item.item, item.qty || ‘’, item.notes || ‘’].map(function(text) {
return new D.TableCell({ children: [new D.Paragraph({ children: [new D.TextRun({ text: text, size: 20, font: ‘Calibri’ })] })], borders: borders });
})});
});
const doc = new D.Document({ sections: [{ properties: {}, children: [
new D.Paragraph({ children: [new D.TextRun({ text: ‘Shopping List – AU Family Dinner Planner’, bold: true, size: 32, font: ‘Calibri’ })], heading: D.HeadingLevel.HEADING_1, spacing: { after: 200 } }),
new D.Paragraph({ children: [new D.TextRun({ text: ’Generated: ’ + new Date().toLocaleDateString(‘en-AU’), size: 18, font: ‘Calibri’, color: ‘888888’ })], spacing: { after: 300 } }),
new D.Table({ rows: [headerRow].concat(rows), width: { size: 100, type: D.WidthType.PERCENTAGE } })
]}]});
D.Packer.toBlob(doc).then(function(blob) { saveAs(blob, ‘shopping-list.docx’); toast(‘Shopping list exported’, ‘success’); });
}
...
const allMeals = [].concat(plan.week1, plan.week2).filter(Boolean);
const children = [
new D.Paragraph({ children: [new D.TextRun({ text: ‘Recipe Pack – AU Family Dinner Planner’, bold: true, size: 36, font: ‘Calibri’ })], heading: D.HeadingLevel.TITLE, spacing: { after: 200 } }),
new D.Paragraph({ children: [new D.TextRun({ text: ‘Fortnight starting ’ + new Date().toLocaleDateString(‘en-AU’), size: 22, font: ‘Calibri’, color: ‘888888’ })], spacing: { after: 400 } }),
new D.Paragraph({ children: [new D.TextRun({ text: ‘Table of Contents’, bold: true, size: 28, font: ‘Calibri’ })], heading: D.HeadingLevel.HEADING_1, spacing: { after: 200 } })
];
allMeals.forEach(function(meal, i) {
children.push(new D.Paragraph({ children: [new D.TextRun({ text: (i+1) + ‘. ’ + meal.title + ’ – ’ + getDifficultyLabel(meal.difficulty) + ’ – ’ + meal.cookTime + ’ min’, size: 22, font: ‘Calibri’ })], spacing: { after: 80 } }));
});
allMeals.forEach(function(meal) {
children.push(
new D.Paragraph({ children: [new D.TextRun({ text: ‘’, break: 1 })], pageBreakBefore: true }),
new D.Paragraph({ children: [new D.TextRun({ text: meal.title, bold: true, size: 30, font: ‘Calibri’ })], heading: D.HeadingLevel.HEADING_1, spacing: { after: 100 } }),
new D.Paragraph({ children: [new D.TextRun({ text: getDifficultyLabel(meal.difficulty) + ’ – ’ + meal.cuisine + ’ – ’ + meal.cookTime + ’ min’ + (meal._aiGenerated ? ’ – AI Generated’ : ‘’), size: 20, font: ‘Calibri’, color: ‘888888’ })], spacing: { after: 200 } }),
new D.Paragraph({ children: [new D.TextRun({ text: ‘Ingredients’, bold: true, size: 24, font: ‘Calibri’ })], heading: D.HeadingLevel.HEADING_2, spacing: { after: 100 } })
);
meal.ingredients.forEach(function(ing) { children.push(new D.Paragraph({ children: [new D.TextRun({ text: ’- ’ + ing, size: 20, font: ‘Calibri’ })], spacing: { after: 40 } })); });
children.push(new D.Paragraph({ children: [new D.TextRun({ text: ‘Method’, bold: true, size: 24, font: ‘Calibri’ })], heading: D.HeadingLevel.HEADING_2, spacing: { before: 200, after: 100 } }));
meal.steps.forEach(function(step, si) { children.push(new D.Paragraph({ children: [new D.TextRun({ text: (si+1) + ’. ’ + step, size: 20, font: ‘Calibri’ })], spacing: { after: 80 } })); });
[[‘meatTip’,‘Meat Prep Tip’,‘C77D0A’],[‘toddlerAdjust’,‘Toddler Adjustment’,‘2E6B8A’],[‘tips’,‘Tips’,‘3D7A3F’],[‘leftoverHandling’,‘Leftover Handling’,‘C2703A’]].forEach(function(sec) {
if (meal[sec[0]]) {
children.push(new D.Paragraph({ children: [new D.TextRun({ text: sec[1], bold: true, size: 20, font: ‘Calibri’, color: sec[2] })], spacing: { before: 200, after: 60 } }));
children.push(new D.Paragraph({ children: [new D.TextRun({ text: meal[sec[0]], size: 20, font: ‘Calibri’, italics: true })], spacing: { after: 100 } }));
}
});
});
const doc = new D.Document({ sections: [{ children: children }] });
D.Packer.toBlob(doc).then(function(blob) { saveAs(blob, ‘recipe-pack.docx’); toast(‘Recipe pack exported’, ‘success’); });
}
...
if (state.currentSession) { state.sessions.push(Object.assign({}, state.currentSession, { archived: true, archivedAt: Date.now() })); save(‘sessions’, state.sessions); }
state.currentSession = { id: generateId(), timestamp: Date.now(), delivery: delivery, meat: meat, leftovers: leftovers, dislikes: dislikesText, outputs: {} };

generateStorageTable(delivery.concat(leftovers));
...
const allMeals = [].concat(state.currentMealPlan.week1, state.currentMealPlan.week2).filter(Boolean);
allMeals.forEach(function(m) { state.mealHistory.push({ date: Date.now(), cuisine: m.cuisine, mexicanFlag: m.cuisine === ‘Mexican’, recipeId: m.id }); });
...
meal.ingredients.forEach(function(ing) {
state.pantry.forEach(function(p) { if (ing.toLowerCase().includes(p.item.toLowerCase()) && !p.have) neededStaples.add(p.item); });
});
});
neededStaples.forEach(function(name) {
const idx = state.pantry.findIndex(function(p) { return p.item === name; });
if (idx >= 0) { state.pantry[idx].low = true; state.pantry[idx]._new = true; }
});
save(‘pantry’, state.pantry);
}
...
navigator.clipboard.writeText(text).then(function() { toast(‘Copied to clipboard’, ‘success’); }).catch(function() {
...
document.getElementById(‘addPantryItemBtn’).addEventListener(‘click’, function() { document.getElementById(‘pantryModal’).style.display = ‘flex’; });
document.getElementById(‘closePantryModal’).addEventListener(‘click’, function() { document.getElementById(‘pantryModal’).style.display = ‘none’; });
document.getElementById(‘savePantryItemBtn’).addEventListener(‘click’, addPantryItem);
document.getElementById(‘closeCookMode’).addEventListener(‘click’, function() { document.getElementById(‘cookModeModal’).style.display = ‘none’; });
...
document.getElementById(‘copyPlainBtn’).addEventListener(‘click’, function() { copyToClipboard(‘wooliesPlainText’); });
document.getElementById(‘copyJSONBtn’).addEventListener(‘click’, function() { copyToClipboard(‘wooliesJSON’); });
document.getElementById(‘copyPromptBtn’).addEventListener(‘click’, function() { copyToClipboard(‘wooliesPrompt’); });
...
if (state.currentSession) generateStorageTable([].concat(state.currentSession.delivery || [], state.currentSession.leftovers || []));
renderPantry();
}
